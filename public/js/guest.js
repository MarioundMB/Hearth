    ...
    <a class="app-card" href="${esc(app.url)}" target="_blank" rel="noopener">
      <div class="app-top">
        ...
        <div>
          ...
          <span class="pill ${app.running ? 'running' : 'stopped'}"><span class="dot"></span>${app.running ? 'läuft' : 'gestoppt'}</span>
        </div>
      </div>
      <div class="app-desc">${esc(app.description)}</div>
      <div class="app-foot">
        <span class="mono muted" style="font-size:12px">:${app.ports.join(' :')}</span>
        <span class="open">Öffnen
  ...
  try {
    ...
    if (!data.apps.length) {
      grid.innerHTML = `<div class="empty"><div class="big">◌</div>
        Keine veröffentlichten Dienste gefunden.<br>
        <span class="muted" style="font-size:13px">Container mit nach außen gemappten Ports erscheinen hier automatisch.</span></div>`;
      stats.innerHTML = '';
      return;
    }
    ...
    stats.innerHTML = `
      <span class="pill running"><span class="dot"></span>${data.apps.length} Dienste online</span>
      <span class="pill"><span class="dot"></span>Host: ${esc(data.host)}</span>`;
  } catch (e) {
    grid.innerHTML = `<div class="empty"><div class="big">⚠</div>Konnte Dienste nicht laden.<br><span class="muted">${esc(e.message)}</span></div>`;
  }
}
